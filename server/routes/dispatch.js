...
router.get('/ready-lots', authenticateToken, async (req, res) => {
  const r = await pool.query(
    `SELECT fg.*, l.lot_no, f.fabric_name, s.shade_name, jo.job_order_no, p.trade_name as party_name
     FROM finished_goods_inventory fg JOIN lots l ON fg.lot_id = l.lot_id
     JOIN fabrics f ON fg.fabric_id = f.fabric_id LEFT JOIN shades s ON fg.shade_id = s.shade_id
     JOIN job_orders jo ON fg.job_order_id = jo.job_order_id JOIN parties p ON jo.party_id = p.party_id
     WHERE fg.tenant_id = $1`, [req.user.tenant_id]);
  res.json(r.rows);
});

router.get('/challans', authenticateToken, async (req, res) => {
  const r = await pool.query(
    `SELECT dc.*, p.trade_name as party_name FROM dispatch_challans dc
     JOIN parties p ON dc.party_id = p.party_id WHERE dc.tenant_id = $1 ORDER BY dc.challan_id DESC`,
    [req.user.tenant_id]);
  res.json(r.rows);
});
...
router.post('/challans', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const no = await nextDocNo(req.user.tenant_id, 'DC', 'dispatch_challans', 'challan_no');
    let totalM = 0, totalKg = 0;
    for (const l of b.lines) { totalM += parseFloat(l.qty_meters); totalKg += parseFloat(l.qty_kg || 0); }
    const dc = await client.query(
      ...
      [req.user.tenant_id, no, b.job_order_id, b.party_id, b.transporter_id, b.vehicle_no, b.lr_no, b.lr_date, totalM, totalKg, req.user.user_id]
    );
    ...
    await client.query(`UPDATE job_orders SET status = 'PARTIALLY_DISPATCHED' WHERE job_order_id = $1`, [b.job_order_id]);
...
router.post('/gst/calculate-tax', authenticateToken, async (req, res) => {
  const tenant = await pool.query(`SELECT state_code FROM tenants WHERE tenant_id = $1`, [req.user.tenant_id]);
  const party = await pool.query(`SELECT state_code FROM parties WHERE party_id = $1`, [req.body.party_id]);
...
router.post('/gst/invoices', authenticateToken, async (req, res) => {
  const b = req.body;
  const tenant = await pool.query(`SELECT state_code FROM tenants WHERE tenant_id = $1`, [req.user.tenant_id]);
  const party = await pool.query(`SELECT state_code FROM parties WHERE party_id = $1`, [b.party_id]);
  const tax = calculateGstTax(party.rows[0]?.state_code || '24', tenant.rows[0]?.state_code || '24', b.lines);
  const no = await nextDocNo(req.user.tenant_id, 'INV', 'gst_invoices', 'invoice_no');
  const inv = await pool.query(
    ...
    [req.user.tenant_id, no, b.invoice_type || 'JOB_WORK_TAX_INVOICE', b.job_order_id, b.party_id, tax.taxable_value, tax.cgst_amount, tax.sgst_amount, tax.igst_amount, tax.total_amount, req.user.user_id]
  );
...
router.get('/gst/invoices', authenticateToken, async (req, res) => {
  const r = await pool.query(`SELECT gi.*, p.trade_name as party_name FROM gst_invoices gi JOIN parties p ON gi.party_id = p.party_id WHERE gi.tenant_id = $1 ORDER BY gi.invoice_id DESC`, [req.user.tenant_id]);
  res.json(r.rows);
});
