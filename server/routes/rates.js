...
router.get('/', authenticateToken, async (req, res) => {
  try {
    const q = `
      ...
      FROM rate_master rm
      LEFT JOIN parties p ON rm.party_id = p.party_id AND p.tenant_id = rm.tenant_id
      LEFT JOIN fabrics f ON rm.fabric_id = f.fabric_id AND f.tenant_id = rm.tenant_id
...
router.post('/calculate', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    `;
    const result = await pool.query(q, [req.tenant_id, process_name, qty || 0]);
