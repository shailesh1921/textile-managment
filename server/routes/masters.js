...
router.get('/me', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT u.user_id, u.username, u.full_name, u.tenant_id, r.role_code, r.role_name as role, t.mill_name
       FROM users u JOIN roles r ON u.role_id = r.role_id JOIN tenants t ON u.tenant_id = t.tenant_id
       WHERE u.user_id = $1`,
      [req.user.user_id]
    );
...
router.post('/parties', authenticateToken, async (req, res) => {
  const b = req.body;
  try {
    const count = await pool.query(`SELECT COUNT(*) FROM parties WHERE tenant_id = $1`, [req.user.tenant_id]);
    ...
    const result = await pool.query(
      ...
      [
        req.user.tenant_id, code, b.party_type, b.legal_name, b.trade_name, b.gstin, b.pan,
...
router.get('/fabrics', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM fabrics WHERE tenant_id = $1 ORDER BY fabric_id`, [req.user.tenant_id]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/fabrics', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [
        req.user.tenant_id, b.fabric_code, b.fabric_name, b.fabric_category || 'WOVEN',
...
router.get('/shades', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT s.*, p.trade_name as customer_name FROM shades s
       LEFT JOIN parties p ON s.customer_party_id = p.party_id
       WHERE s.tenant_id = $1 ORDER BY shade_id`,
      [req.user.tenant_id]
    );
    res.json(result.rows);
  } catch (err) {
...
router.post('/shades', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [req.user.tenant_id, b.shade_card_no, b.shade_name, b.lab_l, b.lab_a, b.lab_b, b.delta_e_tolerance || 1.0, b.customer_party_id]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    const result = await pool.query(
      `SELECT d.*, COALESCE(SUM(s.qty_on_hand),0) as current_stock, p.trade_name as supplier_name
       ...
       LEFT JOIN dye_chemical_stock_batches s ON d.item_id = s.item_id
       LEFT JOIN parties p ON d.preferred_supplier_id = p.party_id
...
router.post('/dye-chemicals', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [req.user.tenant_id, b.item_code, b.item_name, b.category, b.uom || 'KG', b.hsn_code, b.gst_rate_pct || 18, b.reorder_level || 0, b.reorder_qty || 0, b.preferred_supplier_id]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/machines', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM machines WHERE tenant_id = $1 ORDER BY machine_id`, [req.user.tenant_id]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.patch('/machines/:id/status', authenticateToken, async (req, res) => {
  ...
  try {
    const old = await pool.query(`SELECT current_status FROM machines WHERE machine_id = $1 AND tenant_id = $2`, [req.params.id, req.user.tenant_id]);
    const result = await pool.query(
...
router.get('/process-templates', authenticateToken, async (req, res) => {
  try {
    const templates = await pool.query(
      `SELECT pt.*, f.fabric_name FROM process_templates pt LEFT JOIN fabrics f ON pt.fabric_id = f.fabric_id WHERE pt.tenant_id = $1`,
      [req.user.tenant_id]
    );
...
router.get('/recipes', authenticateToken, async (req, res) => {
  try {
    const recipes = await pool.query(
      `SELECT r.*, s.shade_name, f.fabric_name FROM recipes r
       LEFT JOIN shades s ON r.shade_id = s.shade_id LEFT JOIN fabrics f ON r.fabric_id = f.fabric_id
       WHERE r.tenant_id = $1 ORDER BY recipe_id`,
      [req.user.tenant_id]
    );
    for (const r of recipes.rows) {
      const lines = await pool.query(
        `SELECT rl.*, d.item_name, d.item_code FROM recipe_lines rl JOIN dye_chemicals d ON rl.item_id = d.item_id WHERE rl.recipe_id = $1 ORDER BY rl.sequence_no`,
        [r.recipe_id]
      );
      r.lines = lines.rows;
    }
