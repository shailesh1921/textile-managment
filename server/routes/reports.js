...
router.get('/daily-production', authenticateToken, async (req, res) => {
  const r = await pool.query(
    `SELECT pe.shift_date, pe.shift, m.machine_name, m.machine_type, SUM(pe.output_meters) as output_meters, SUM(pe.in_process_loss_pct) as loss
     FROM production_entries pe JOIN machines m ON pe.machine_id = m.machine_id
     WHERE m.tenant_id = $1 GROUP BY pe.shift_date, pe.shift, m.machine_name, m.machine_type ORDER BY pe.shift_date DESC LIMIT 50`,
    [req.user.tenant_id]);
  res.json(r.rows);
});
...
router.get('/consumption-variance', authenticateToken, async (req, res) => {
  const r = await pool.query(
    `SELECT d.item_name, SUM(rdl.standard_qty) as standard_qty, SUM(rdl.actual_qty) as actual_qty,
     ...
     FROM recipe_dispensing_logs rdl JOIN dye_chemicals d ON rdl.item_id = d.item_id
     JOIN batch_runs br ON rdl.batch_id = br.batch_id WHERE br.tenant_id = $1 GROUP BY d.item_name`,
    [req.user.tenant_id]);
  res.json(r.rows);
});
...
router.get('/machine-utilization', authenticateToken, async (req, res) => {
  const r = await pool.query(
    `SELECT m.machine_name, m.machine_type, m.current_status,
     ...
     FROM machines m LEFT JOIN batch_runs br ON m.machine_id = br.machine_id
