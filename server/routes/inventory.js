...
router.get('/grey-fabric', authenticateToken, async (req, res) => {
  ...
  try {
    let q = `SELECT g.*, p.trade_name as party_name, f.fabric_name, l.lot_no, jo.job_order_no
             ...
             JOIN parties p ON g.party_id = p.party_id JOIN fabrics f ON g.fabric_id = f.fabric_id
             LEFT JOIN lots l ON g.lot_id = l.lot_id LEFT JOIN job_orders jo ON g.job_order_id = jo.job_order_id
             WHERE g.tenant_id = $1`;
  ...
  try {
    const result = await pool.query(
      `SELECT s.*, d.item_name, d.item_code, d.category, d.reorder_level, p.trade_name as supplier_name
       FROM dye_chemical_stock_batches s JOIN dye_chemicals d ON s.item_id = d.item_id
       LEFT JOIN parties p ON s.supplier_id = p.party_id WHERE s.tenant_id = $1 ORDER BY s.expiry_date`,
      [req.user.tenant_id]
    );
    res.json(result.rows);
  } catch (err) {
  ...
  try {
    const result = await pool.query(
      `SELECT fg.*, l.lot_no, f.fabric_name, s.shade_name, jo.job_order_no, p.trade_name as party_name
       ...
       JOIN lots l ON fg.lot_id = l.lot_id JOIN fabrics f ON fg.fabric_id = f.fabric_id
       LEFT JOIN shades s ON fg.shade_id = s.shade_id JOIN job_orders jo ON fg.job_order_id = jo.job_order_id
       JOIN parties p ON jo.party_id = p.party_id WHERE fg.tenant_id = $1`,
      [req.user.tenant_id]
    );
    res.json(result.rows);
  } catch (err) {
  ...
  try {
    const chemicals = await pool.query(
      `SELECT d.item_id, d.item_code, d.item_name, d.category, d.reorder_level, COALESCE(SUM(s.qty_on_hand),0) as current_stock
       FROM dye_chemicals d LEFT JOIN dye_chemical_stock_batches s ON d.item_id = s.item_id
    ...
    const grey = await pool.query(`SELECT COUNT(*) as lots, COALESCE(SUM(qty_meters),0) as meters FROM grey_fabric_inventory WHERE tenant_id = $1`, [req.user.tenant_id]);
    const fg = await pool.query(`SELECT COUNT(*) as lots, COALESCE(SUM(qty_meters),0) as meters FROM finished_goods_inventory WHERE tenant_id = $1`, [req.user.tenant_id]);
...
router.get('/movements', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT sm.*, u.username FROM stock_movements sm LEFT JOIN users u ON sm.created_by = u.user_id
...
router.post('/grn', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const grnNo = await nextDocNo(req.user.tenant_id, 'GRN', 'grn_records', 'grn_number');
    let total = 0;
    const grn = await client.query(
      ...
      [req.user.tenant_id, grnNo, po_id, supplier_id, req.user.user_id]
    );
    for (const item of items) {
      ...
      await client.query(
        ...
        [req.user.tenant_id, item.item_id, item.batch_lot_no, item.qty, item.unit_cost, item.expiry_date, supplier_id, item.location || 'Chem Store']
      );
      await client.query(
        ...
        [req.user.tenant_id, item.item_id, grn.rows[0].grn_id, item.qty, item.unit_cost, req.user.user_id]
      );
    }
    await client.query(`UPDATE grn_records SET total_amount = $1 WHERE grn_id = $2`, [total, grn.rows[0].grn_id]);
...
router.get('/packing', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT pm.*, COALESCE(ps.qty_on_hand,0) as qty_on_hand FROM packing_materials pm
       LEFT JOIN packing_stock ps ON pm.packing_id = ps.packing_id WHERE pm.tenant_id = $1`,
      [req.user.tenant_id]
    );
    res.json(result.rows);
  } catch (err) {
